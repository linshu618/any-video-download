...
test('YouTube downloads resolve the page again rather than reusing a stale HLS URL',()=>{
 ...
 assert.match(args[args.indexOf('-f')+1],/height=1080/);assert.match(args[args.indexOf('-f')+1],/protocol\^=m3u8/);assert.ok(args.includes('--force-ipv4'));assert.ok(!args.some(x=>x.includes('stale.test')));assert.ok(args.some(x=>x.includes('%%work')));
});
...
test('Metadata includes HLS and webm heights that can be remuxed to MP4',async()=>{
 const info=await youtubeInfo({ytDlp:'fake.exe'},'https://www.youtube.com/watch?v=6l7ble9P74o',String,fakeInfo({id:'6l7ble9P74o',title:'HLS',duration:19,formats:[
  {ext:'mp4',protocol:'m3u8_native',vcodec:'avc1',acodec:'none',height:1080},{ext:'webm',vcodec:'vp9',acodec:'none',height:720},{ext:'m4a',vcodec:'none',acodec:'aac'},{ext:'mp4',vcodec:'avc1',acodec:'none',height:2160,has_drm:true}]}));
 assert.deepEqual(info.heights,[1080,720]);
});
test('Metadata rejects a different video and no usable tracks',async()=>{
