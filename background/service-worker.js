...
function classify(url, type = "") {
  ...
  if (/vnd.yt-ump/i.test(type)) return null;
...
function parseHls(text, url) {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("#EXT-X-MAP:")) {
      const u = httpUrl(attrs(line).URI, url);
    ...
    if (pending) {
      ...
      variants.push({
        ...
        audioUrl: track?.URI ? httpUrl(track.URI, url) : null
      });
      pending = null;
  ...
  return {
    variants: variants.sort((a, b) => (b.height || 0) - (a.height || 0) || b.bandwidth - a.bandwidth),
    related,
    segments,
    duration: duration || null,
    live: variants.length ? null : !lines.includes("#EXT-X-ENDLIST"),
...
function parseDash(text, url) {
  ...
  for (const period of array(mpd.Period)) for (const set of array(period.AdaptationSet)) {
    if (set.ContentProtection !== void 0) protectedMedia = true;
    for (const rep of array(set.Representation)) {
      if (rep.ContentProtection !== void 0) protectedMedia = true;
      if (!(rep.height || set.height || /video/.test(rep.mimeType || set.mimeType || set.contentType || ""))) continue;
      variants.push({
        url,
        representationId: String(rep.id ?? ""),
        height: Number(rep.height || set.height) || null,
        width: Number(rep.width || set.width) || null,
        bandwidth: Number(rep.bandwidth) || 0
      });
    }
  }
  return {
    variants: variants.sort((a, b) => (b.height || 0) - (a.height || 0)),
    duration: seconds(mpd.mediaPresentationDuration),
...
function mergeMedia(rows, incoming) {
  ...
  if (parent) {
    return rows.filter((r) => r.id === parent.id || !item.segments?.includes(r.url)).map((r) => r.id === parent.id ? {
      ...r,
      duration: item.parsed && item.duration ? item.duration : r.duration || item.duration,
      live: item.live ?? r.live,
      segments: [.../* @__PURE__ */ new Set([...r.segments || [], ...item.segments || []])],
      protected: r.protected || item.protected
    } : r);
  }
  if (rows.some((r) => r.segments?.includes(item.url))) return rows;
  const children = rows.filter((r) => item.related?.includes(r.url));
  item.segments = [.../* @__PURE__ */ new Set([...item.segments || [], ...children.flatMap((r) => r.segments || [])])];
...
var KEY = "download_history_v1";
var active = (job) => ["starting", "downloading", "finalizing", "cancelling"].includes(job.status);
var positive = (value) => Number.isFinite(Number(value)) && Number(value) > 0 ? Number(value) : null;
var DownloadManager = class {
  constructor(api) {
    ...
    api.downloads.onChanged.addListener(() => {
      this.list().catch(console.error);
    });
  }
  run(fn) {
    const next = this.queue.catch(() => {
    }).then(fn);
    this.queue = next;
    return next;
  }
  ...
  async refreshFiles() {
    ...
    for (const row of this.rows) if (row.kind === "file" && active(row) && row.browserId != null) {
      ...
      if (!item) Object.assign(row, { status: "interrupted", error: "\u6D4F\u89C8\u5668\u4E0B\u8F7D\u8BB0\u5F55\u5DF2\u4E0D\u5B58\u5728" });
        ...
        row.bytes = item.bytesReceived || 0;
        row.totalBytes = positive(item.totalBytes);
        row.path = item.filename || row.path;
  ...
  start(input) {
    return this.run(async () => {
      ...
      const row = {
        ...
        title: String(input.title || "\u89C6\u9891").slice(0, 300),
        kind: input.kind,
        quality: input.height ? `${input.height}p` : "",
        status: "starting",
        seconds: 0,
        bytes: 0,
        duration: positive(input.duration),
        percent: null,
        speed: null,
        createdAt: Date.now(),
        updatedAt: Date.now()
      ...
      try {
        if (input.kind === "file") {
          ...
          const port = this.api.runtime.connectNative("com.any_video_download.helper");
          this.ports.set(row.id, port);
          port.onMessage.addListener((msg) => {
            this.update(row.id, msg).catch(console.error);
          });
...
// background/sender.js
function isExtensionPage(sender, id) {
...
var key = (id) => `media_v2_${id}`;
async function state(id) {
  return (await chrome.storage.session.get(key(id)))[key(id)] || { epoch: 0, videos: [] };
}
function serial(id, task) {
  const next = (queues.get(id) || Promise.resolve()).catch(() => {
  }).then(task);
  queues.set(id, next);
  next.finally(() => {
...
async function reset(id, pageUrl) {
  return serial(id, async () => {
    ...
    await publish(id, { epoch: s.epoch + 1, pageUrl, videos: [] });
  });
}
async function playlist(url, kind) {
  const response = await fetch(url, { credentials: "include", signal: AbortSignal.timeout(15e3) });
  ...
  while (true) {
    ...
    if (length > 4 * 1024 * 1024) {
      await reader.cancel();
      throw new Error("\u64AD\u653E\u5217\u8868\u8D85\u8FC7 4 MB\uFF0C\u5DF2\u505C\u6B62\u89E3\u6790");
    }
    chunks.push(value);
  }
...
async function discover(tabId, candidate) {
  candidate = Object.fromEntries(Object.entries(candidate).filter(([, value]) => value !== void 0));
  ...
  if (inFlight.has(token)) {
    const flight2 = inFlight.get(token);
    flight2.queued = { ...flight2.queued, ...candidate };
    return;
  }
  const flight = { queued: null, epoch: null };
  ...
  try {
    ...
    await serial(tabId, async () => {
      ...
      const refresh = kind !== "file" && (candidate.forceRefresh || !old?.parsed || Date.now() - (old.parsedAt || 0) > 1e4);
      ...
      const item = {
        ...
        kind,
        title: candidate.title || old?.title || tab.title || "\u89C6\u9891",
        pageUrl: tab.url,
        size: kind === "file" ? candidate.size || old?.size || null : null,
        timestamp: old?.timestamp || Date.now()
      };
      needsParse = refresh;
      s.videos = mergeMedia(s.videos, item).slice(-200);
      s.pageUrl = tab.url;
    ...
    try {
      ...
      if (kind === "hls" && parsed.variants.length) {
        try {
          ...
          parsed.duration = child.duration;
          parsed.live = child.live;
          parsed.segments = child.segments;
          parsed.protected ||= child.protected;
        } catch {
        }
      }
      parsed.parsed = true;
      parsed.parsedAt = Date.now();
      parsed.error = null;
    } catch (error) {
      parsed = { error: error.message, parsed: false };
    }
    await serial(tabId, async () => {
      const s = await state(tabId);
...
chrome.webRequest.onResponseStarted.addListener((details) => {
  ...
  const kind = classify(details.url, type);
  if (!kind) return;
...
chrome.runtime.onMessage.addListener((msg, sender, reply) => {
  ...
  if (msg.type === "PLAYER_STATE" && sender.tab) {
    serial(sender.tab.id, async () => {
      ...
      const incoming = (Array.isArray(msg.players) ? msg.players : []).slice(0, 20).map((p) => ({
        ...
        frameId,
        src: typeof p.src === "string" ? p.src.slice(0, 16e3) : "",
  ...
  if (msg.type === "MEDIA_SCAN" && sender.tab) {
    for (const item of (msg.items || []).slice(0, 100)) {
      ...
      discover(sender.tab.id, {
        url: item.url,
        frameId: sender.frameId || 0,
        type: String(item.type || "").slice(0, 100),
        title: typeof item.title === "string" ? item.title.slice(0, 300) : void 0,
        poster: httpUrl(item.poster) || void 0,
        duration: Number(item.duration) || void 0,
        width: Number(item.width) || void 0,
        height: Number(item.height) || void 0
      });
    }
  }
});
